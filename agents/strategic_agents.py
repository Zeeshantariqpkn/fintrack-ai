...
logic when GROQ_API_KEY is missing.
"""
...
def run_analytics_agent(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    evidence.append(_ev("cashflow_months", f"{positive_months}/{positive_months + negative_months}",
...
def run_risk_agent(stats: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if er > 90:
        risks.append({"title": "Critical Expense Ratio",
            "description": f"Expenses consume {er:.1f}% of revenue — very little margin remains.",
            "severity": "HIGH", "evidence": f"expense_ratio={er:.1f}%"})
        evidence.append(f"expense_ratio={er:.1f}%")
        score += 25
    elif er > 75:
        risks.append({"title": "High Expense Ratio",
            "description": f"Expenses consume {er:.1f}% of revenue.",
            "severity": "MEDIUM", "evidence": f"expense_ratio={er:.1f}%"})
        evidence.append(f"expense_ratio={er:.1f}%")
        score += 15
    elif er > 60:
        risks.append({"title": "Rising Cost Pressure",
            "description": f"Expenses consume {er:.1f}% of income.",
            "severity": "LOW", "evidence": f"expense_ratio={er:.1f}%"})
        evidence.append(f"expense_ratio={er:.1f}%")
        score += 7

    if stats["spending_concentration"] > 0.35 and stats.get("largest_category"):
        risks.append({"title": "Spending Concentration",
            "description": f"{stats['largest_category']} dominates expenses at ${stats['largest_category_value']:,.0f}.",
            "severity": "MEDIUM", "evidence": f"HHI={stats['spending_concentration']:.3f}"})
        evidence.append(f"HHI={stats['spending_concentration']:.3f}")
        score += 12

    if stats["top_vendor_share"] > 25 and stats.get("top_vendor"):
        risks.append({"title": "Vendor Concentration",
            "description": f"{stats['top_vendor']} accounts for {stats['top_vendor_share']:.1f}% of total expenses.",
            "severity": "MEDIUM", "evidence": f"top_vendor_share={stats['top_vendor_share']:.1f}%"})
    ...
    if not reasoning:
        if risks:
            ...
            reasoning = (f"Risk Agent identified {len(risks)} issue(s): {titles}. "
                        f"Combined risk score is {score:.0f}/100, indicating a {level} risk profile.")
        else:
            reasoning = ("Risk Agent found no material risks: cash flow is positive, expense "
...
def _risk_reasoning_llm(risks, stats) -> Optional[str]:
    if not hf_available():
        return None
    prompt = (
...
def run_opportunity_agent(stats: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if stats.get("largest_category") and stats["total_expenses"] > 0:
        ...
        opportunities.append({
            ...
            "description": (f"{stats['largest_category']} is the largest spending category at "
    ...
    if stats.get("top_vendor") and stats["top_vendor_share"] > 10:
        opportunities.append({
            ...
            "description": (f"{stats['top_vendor']} is the highest-spend vendor at "
    ...
    if rec:
        ...
        opportunities.append({
            ...
            "description": (f"{len(rec)} recurring expense pattern(s) detected with an annualized "
                           f"impact of ≈${annual:,.0f}."),
            "impact": "MEDIUM",
    ...
    if stats["net_cash_flow"] > 0 and stats["expense_ratio"] > 50:
        opportunities.append({
            ...
            "description": ("Net cash flow is positive but the expense ratio is elevated. "
                           "Reallocating surplus toward higher-ROI areas could compound growth."),
            "impact": "MEDIUM",
    ...
    if stats["revenue_trend"] == "rising":
        opportunities.append({
            ...
            "description": ("Revenue trend is rising. Investing in the channels driving this "
                           "growth could accelerate the trajectory."),
            "impact": "HIGH", "evidence": "revenue_trend=rising",
        })
    ...
    if stats["spending_concentration"] > 0.3:
        opportunities.append({
            ...
            "description": (f"Expense concentration (HHI {stats['spending_concentration']:.2f}) "
                           "suggests reliance on a few categories."),
            "impact": "LOW",
    ...
    if not reasoning:
        if opportunities:
            reasoning = (f"Opportunity Agent identified {len(opportunities)} action area(s). "
                        f"Highest-leverage: {opportunities[0]['title']}")
        else:
            reasoning = "No significant opportunities detected."
...
def _opportunity_reasoning_llm(opps, stats) -> Optional[str]:
    if not hf_available():
        return None
    prompt = (
        "You are the Opportunity Agent. Given these opportunities and analytics, "
        "write 2 sentences of reasoning. Do not invent numbers.\n\n"
...
# Decision Agent
# =====================================================================
def run_decision_agent(stats, risk, opportunity, revision_hint=None) -> Dict[str, Any]:
    ...
    if risk_pressure >= 0.6 and top_risk:
        ...
        priority = "HIGH"
        decision = (f"Prioritize risk mitigation on '{top_risk['title']}' before pursuing growth. "
                   f"{top_risk['description']}")
        expected = "Reduce financial risk exposure and protect cash position."
        actions = [f"Address: {top_risk['title']}",
        ...
        decision = f"{top_opp['description']} This is the highest-leverage action."
        expected = "Lower operating costs or improve cash-flow efficiency."
        actions = [f"Execute: {top_opp['title']}",
        ...
        priority = "LOW"
        decision = "No material risks or high-impact opportunities detected."
    ...
    if llm_reasoning:
        ...
        reasoning = (f"Risk score is {risk.get('risk_score', 0)}/100 and opportunity score is "
    ...
    result = {
        "title": title, "priority": priority, "decision": decision,
        "reasoning": reasoning, "evidence": evidence,
        "expected_impact": expected, "recommended_actions": actions,
    }
    if revision_hint:
...
def _decision_reasoning_llm(stats, risk, opportunity, revision_hint) -> Optional[str]:
    if not hf_available():
        return None
    prompt = (
        "You are the Decision Agent. Weigh risks and opportunities and justify "
        "(2 sentences) the single most important business action. Use only provided numbers.\n\n"
        f"Analytics: {json.dumps({k: v for k, v in stats.items() if k != 'monthly'})[:1500]}\n"
        f"Risk: {json.dumps(risk)[:1200]}\n"
        f"Opportunity: {json.dumps(opportunity)[:1200]}\n"
    )
    if revision_hint:
        prompt += f"Previous critic feedback: {revision_hint}\n"
    prompt += "Reasoning:"
...
# Critic Agent
# =====================================================================
def run_critic_agent(decision, risk, opportunity, stats) -> Dict[str, Any]:
    problems: List[str] = []
    evidence: List[str] = []
    ...
    if len(decision.get("reasoning", "")) < 60:
    ...
    if llm_verdict and llm_verdict.get("status") in {"APPROVED", "REVISE"}:
        ...
        if approved:
            reasoning_text = ("Critic Agent verified: decision is supported by evidence, "
                            "consistent with risks/opportunities, and actionable.")
...
def _critic_llm(decision, risk, opportunity, stats) -> Optional[dict]:
    if not hf_available():
        return None
    prompt = (
        "You are the Critic Agent. Review the decision below. Respond ONLY with a "
        'JSON object: {"status": "APPROVED" or "REVISE", "reasoning": "...", '
